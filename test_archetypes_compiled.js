const fs = require('fs');
const { ArchetypeEngine } = require('./server/dist/modules/user/archetypeEngine.js');
...
async function run() {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/profiles?select=id,full_name,profile_data,astrology,numerology,mayan`, {
        headers: {
