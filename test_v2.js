...
const runTest = (name, profile, expectedDisplay, expectedReason, expectedSecondary) => {
    const result = ArchetypeEngine.calculate(profile, 'es');
    const v2 = result.assignment_v2;
...
runTest('Missing Color', { astrology: { sunSign: 'Taurus', moonSign: 'Virgo' }, mayan: {} }, 'tierra', 'SINGLE_WINNER', null);
runTest('Tie (Legacy Fuego)', { astrology: { sunSign: 'Aries' }, mayan: { color: 'Azul' } }, 'fuego', 'CO_DOMINANT_LEGACY_CORE_DISPLAY', null);
runTest('Tie (Legacy Tierra)', { astrology: { sunSign: 'Taurus' }, mayan: { color: 'Rojo' } }, 'tierra', 'CO_DOMINANT_LEGACY_CORE_DISPLAY', null);
