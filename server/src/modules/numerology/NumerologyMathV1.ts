export type NumerologyFormulaVersion = "naos_numerology_math_v1";
export type PinnacleIndex = 1 | 2 | 3 | 4;

export interface MasterNumberMetadata {
    number: number;
    reducedBase: number;
    isMaster: boolean;
}
...
export interface PinnacleData {
    value: number;
    metadata: MasterNumberMetadata;
}

export interface CurrentPinnacleConfiguration {
...
export class NumerologyMathV1 {
    static readonly VERSION: NumerologyFormulaVersion = "naos_numerology_math_v1";
     ...
     */
    static reduceNumber(value: number, options: { preserveMasters: boolean } = { preserveMasters: true }): number {
        if (value === 0) return 0;
        let n = value;
        while (n > 9) {
     ...
     */
    static getMasterMetadata(num: number): MasterNumberMetadata {
     ...
     */
    static calculateLifePath(year: number, month: number, day: number): number {
     ...
     */
    static calculatePinnacles(year: number, month: number, day: number): [number, number, number, number] {
     ...
     */
    static getCurrentPinnacle(birthYear: number, birthMonth: number, birthDay: number, currentDateStr: string): CurrentPinnacleConfiguration {
        ...
        let index: PinnacleIndex = 1;
        let pNum = pinnacles[0];
        let startD = birthDate;
        let endD: Date | null = p1EndDate;
        let startA = 0;
        let endA: number | null = p1EndAge;
     ...
     */
    static calculateUniversalCycles(currentYear: number, currentMonth: number, currentDay: number) {
    ...
    static calculatePersonalCycles(birthMonth: number, birthDay: number, currentYear: number, currentMonth: number, currentDay: number) {
        const uy = this.reduceNumber(currentYear, { preserveMasters: true });
