...
function updateFile(filepath, interpretations, isSpanish) {
    let content = fs.readFileSync(filepath, 'utf8');

    if (isSpanish && !content.includes('interpretacion_profunda?: string;')) {
        content = content.replace('descripcion: string;', 'descripcion: string;\n    interpretacion_profunda?: string;');
    }

    for (const [id, interpretation] of Object.entries(interpretations)) {
        const regex = new RegExp(`(id:\\s*['"]${id}['"].*?descripcion:\\s*["'].*?["'],)`, 'gs');
        content = content.replace(regex, (match, p1) => {
            if (!p1.includes('interpretacion_profunda:')) {
...
const basePath = path.join('c:', 'Users', 'l_her', 'naos-platform', 'client', 'src', 'constants');
updateFile(path.join(basePath, 'archetypeData.ts'), es_interpretations, true);
updateFile(path.join(basePath, 'archetypeData_en.ts'), en_interpretations, false);
